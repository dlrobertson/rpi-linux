...
static void bma400_output_data_rate_from_raw(int idx, unsigned int *val,
					     unsigned int *val2)
{
	*val = BMA400_ACC_ODR_MAX_HZ >> (BMA400_ACC_ODR_MAX_RAW - idx);
	if (idx > BMA400_ACC_ODR_MIN_RAW)
		*val2 = 0;
	else
		*val2 = 500000;
}
...
static int bma400_get_accel_output_data_rate(struct bma400_data *data)
{
	...
	case POWER_MODE_LOW:
		 ...
		 * in the datasheet.
		 */
		bma400_output_data_rate_from_raw(BMA400_ACC_ODR_MIN_RAW,
						 &data->sample_freq.hz,
...
static int bma400_set_accel_output_data_rate(struct bma400_data *data,
					     int hz, int uhz)
{
	unsigned int idx = BMA400_ACC_ODR_MIN_RAW;
	unsigned int odr;
	unsigned int val;
	int ret;

	if (hz != BMA400_ACC_ODR_MIN_HZ) {
		if (hz > BMA400_ACC_ODR_MAX_HZ || hz % 25 || uhz)
			return -EINVAL;

		while (hz > BMA400_ACC_ODR_MIN_HZ) {
			idx += 1;
			hz >>= 1;
		}
	} else if (uhz != 500000) {
